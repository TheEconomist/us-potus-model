...
# model version final:
# source('scripts/R/Backtesting/final_2008.R');source('scripts/R/Backtesting/final_2012.R');source('scripts/model/poll_model_2020.R');rmarkdown::render('model_summary.Rmd',output_file = 'model_reports/final_final_actual_new_prior_optimized_covar_0.6cor_newcovmat_cor_average_student_t.html');beepr::beep(2);
```
...
# national
p_clinton_natl <- pblapply(1:dim(predicted_score)[2],
                         function(x){
                           # each row is a day for a particular draw
                           temp <- predicted_score[x,,] %>% as.data.frame()
                           names(temp) <- colnames(state_correlation)
