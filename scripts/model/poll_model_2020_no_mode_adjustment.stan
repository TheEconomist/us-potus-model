...
transformed parameters {
  ...
  for (t in 1:(T - current_T)) mu_b[:, T - t] = cholesky_ss_cov_mu_b_walk * raw_mu_b[:, T - t] + mu_b[:, T - t + 1];
  for (t in 1:(current_T - 1)) mu_b[:, current_T - t] = cholesky_ss_cov_mu_b_walk * raw_mu_b[:, current_T - t] + mu_b[:, current_T - t + 1];
