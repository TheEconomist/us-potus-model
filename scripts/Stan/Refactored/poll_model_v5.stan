data{
  ...
  int S;    // Number of states (for which at least 1 poll is available)
  int P;    // Number of pollsters
  int<lower = 1, upper = S> state[N_state]; // State index
  int<lower = 1, upper = T> day_state[N_state];   // Day index
  int<lower = 1, upper = T> day_state[N_national];   // Day index
