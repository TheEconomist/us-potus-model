...
output_list <- read_rds("test_mu_d_stan_model_2016-11-08.rds")

model <- rstan::stan_model("scripts/Stan/Adjusted_v_unadjusted/poll_model_adj_v_nadj_v1.stan")
#output_list <- list()
values_mu_d_sigma <- c(0.01, 0.02, 0.03, 0.04)
len_val_mu_d_sigma <- length(values_mu_d_sigma)

for (i_val in 3:len_val_mu_d_sigma){
  output_list[[i_val]] <- list()
  output_list[[i_val]][["val_sigma"]] <- values_mu_d_sigma[[i_val]]
  ...
  prior_sigma_d <- values_mu_d_sigma[[i_val]]
  ...
  # mu_d
  mu_d <- rstan::extract(out, pars = "mu_d")[[1]]
  mu_d_plot <- lapply(1:100,
         function(x){
           tibble(mu_d_draw = inv.logit(mu_d[x,]),
                  trial = x) %>%
             mutate(date = min(df$end) + row_number()) 
         }) %>%
    do.call('bind_rows',.) %>%
    ggplot(.,aes(x=date,y=mu_d_draw,group=trial)) +
    geom_line(alpha=0.2)
  
  output_list[[i_val]][["mu_d_plot"]] <- mu_d_plot
  ...
  output_list[[i_val]][["plt_joint"]] <- plt_joint
  ...
  diff_clinton <- p_clinton[p_clinton$state != '--',] %>%
  ...
  output_list[[i_val]][["diff_clinton"]] <- diff_clinton
  ...
  ev_plt <- ggplot(final_evs,aes(x=dem_ev,
  ...
  output_list[[i_val]][["ev_plt"]] <- ev_plt
  ...
  brier <- tibble(outlet = c('538 polls-plus','538 polls-only','princeton','nyt upshot','kremp/slate','pollsavvy','predictwise markets','predictwise overall','desart and holbrook','daily kos','huffpost'),
  ...
  output_list[[i_val]][["brier"]] <- brier
}

write_rds(output_list, sprintf('test_mu_d_stan_model_%s.rds',RUN_DATE),compress = 'gz')

## joint
grid.arrange(output_list[[1]][["plt_joint"]])
grid.arrange(output_list[[2]][["plt_joint"]])
grid.arrange(output_list[[3]][["plt_joint"]])
grid.arrange(output_list[[4]][["plt_joint"]])

## state level
grid.arrange(output_list[[1]][["diff_clinton"]])
grid.arrange(output_list[[2]][["diff_clinton"]])
grid.arrange(output_list[[3]][["diff_clinton"]])
grid.arrange(output_list[[4]][["diff_clinton"]])

## electoral college votes
grid.arrange(output_list[[1]][["ev_plt"]])
grid.arrange(output_list[[2]][["ev_plt"]])
grid.arrange(output_list[[3]][["ev_plt"]])
grid.arrange(output_list[[4]][["ev_plt"]])

## mu_dplot
grid.arrange(output_list[[1]][["mu_d_plot"]])
grid.arrange(output_list[[2]][["mu_d_plot"]])
grid.arrange(output_list[[3]][["mu_d_plot"]])
grid.arrange(output_list[[4]][["mu_d_plot"]])
