...
NATIONAL_PRIOR_DEM_TWO_PARTY_SHARE <- 0.51

# get results
results <- politicaldata::pres_results #%>% filter(year < 2016)
...
# compare with loess
predictions %>%
  filter(state %in% c('FL','NC','WI','PA','MI','MN','AZ','NH')) %>%
  ggplot(.,
       aes(x=year,col=state,group=state)) +
  geom_hline(yintercept = 0) +
  geom_point(aes(y=dem_two_party_share_lean)) + 
  geom_line(aes(y=dem_two_party_share_lean)) +
