...
prior_sigma_a_list <- c(0.02, 0.03, 0.04)
prior_sigma_b_list <- c(0.02, 0.03, 0.04)
df_save <- list()
count <- 0
for (i in 1:3){
  for (j in 1:3){
    ...
    data <- list(
      ...
      prior_sigma_a = prior_sigma_a_list[i],
      prior_sigma_b = prior_sigma_b_list[j],
      mu_b_prior = mu_b_prior,
    ...
    # read model code
    model <- rstan::stan_model("scripts/Stan/Refactored/poll_model_v11.stan")
    ...
    # llh
    df_save[[count]] <- list(loo = loo(out, pars = "llh"), prior_sigma_a = prior_sigma_a_list[i], prior_sigma_b = prior_sigma_b_list[j])
  }
}
