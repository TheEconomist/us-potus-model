...
for (i_mu_b_walk in 1:len_mu_b_walk_sd){
  ...
  plt <- grid.arrange(natl_polls.gg, natl_evs.gg, state_polls.gg, 
               layout_matrix = rbind(c(1,1,3,3,3),
                                     c(2,2,3,3,3)),
               top = identifier
  )
  output_list[[i_mu_b_walk]][["joint_plt"]] <- plt
  ...
  plt_states <- p_clinton %>%
    ...
    theme_minimal()  +
    theme(legend.position = 'none') +
    scale_x_date(limits=c(ymd('2016-09-01','2016-11-08')),date_breaks='1 month',date_labels='%b') +
    scale_y_continuous(breaks=seq(0,1,0.1)) +
    labs(subtitle = identifier)

  output_list[[i_mu_b_walk]][["by_states"]] <- plt_states
  ...
  plt_final_evs <- ggplot(final_evs,aes(x=dem_ev,
  ...
  output_list[[i_mu_b_walk]][["plt_final_evs"]] <- plt_final_evs
...
## joint
grid.arrange(output_list[[1]][["joint_plt"]])
grid.arrange(output_list[[4]][["joint_plt"]])
