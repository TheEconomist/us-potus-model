data{
  ...
  vector[S] state_weights; // based on 2012/16 turnouts
  // data
  int n_democrat[N];
  int n_respondents[N];
  int n_two_share[N];
...
parameters {
  ...
  // eta
  matrix[2, N] raw_eta;
  cholesky_factor_corr[2] chol_eta;
}
...
transformed parameters {
  ...
  // eta
  for (n in 1:N) eta[:,n] = chol_eta * raw_eta[:,n]; 
  // averages
  ...
  for (i in 1:N){
    ...
    if (state[i] == 52){
      ...
      pi_democrat[i] = mu_a[day[i]] + logit(sum_average_states[day[i]]) + alpha + mu_c[poll[i]] + 
        sigma_measure_noise_national * measure_noise[i] + eta[2, i];
      pi_two_share[i] = eta[1, i];
    } else {
      // state-level
      ...
      pi_democrat[i] = mu_a[day[i]] + mu_b[state[i], day[i]] + mu_c[poll[i]] + 
        measure_noise[i] * sigma_measure_noise_state + polling_error[state[i]] + eta[2, i];
      pi_two_share[i] = eta[1, i];
    }
  }
}
...
model {
  ...
  // raw_eta
  for (i in 1:2) raw_eta[i] ~ std_normal();
  chol_eta ~ lkj_corr_cholesky(prior_chol_eta);
  //*** likelihood
  n_two_share ~ binomial_logit(n_respondents, pi_two_share);
  n_democrat ~ binomial_logit(n_two_share, pi_democrat);
}
